"""SourceAdapter for ASP.NET/AJAX platform scraper data."""
...
class AspAdapter(SourceAdapter):
    """Transforms raw dicts from AspAjaxScraper into CanonicalRecords.

    Handles data from: J. Morais Imoveis.
    """

    source_type = "asp_ajax"
    ...
    def transform(self, raw: dict[str, Any]) -> CanonicalRecord:
        return CanonicalRecord(
            ...
            potencia_eletrica_kva=_i(raw.get("eletrica")),
...
def _extract_price(raw: dict, op: str) -> float | None:
    ...
    if op == "locacao" and ("mês" in lower or "/m" in lower):
        return val
